...
def merge_xlsx_files(folder_path):
    """
    Merges all XLSX files in a specified folder into a single pandas DataFrame,
    but only if they all have the exact same columns.
    ...
    for xlsx_file in xlsx_files[1:]:
        ...
        try:
            ...
            if current_cols != reference_cols:
                raise ValueError(
                    f"""Column mismatch! '{xlsx_file}' has columns {current_cols},
                    but expected columns are {reference_cols}."""
                )
